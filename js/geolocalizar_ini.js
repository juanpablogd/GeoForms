...
var geoexitoso = function(pos) {
	...
	heading = pos.coords.heading;	if(heading == null) heading = "";		 	//text = "<div>Latitude: " + myLatitud + "<br/>" + "Longitude: " + myLongitud + "<br/>" + "Accuracy: " + myPrecision + " m<br/>" + "</div>";
	 ...
	 timestamp = new Date(pos.timestamp);	timestamp = timestamp.format("yyyy-mm-dd HH:mm:ss");
	 text = '<table class="table table-striped"><tr><td>Latitud:</td><td>' + myLatitud + '</td></tr><tr><td>Longitud:</td><td>' + myLongitud + '</td></tr><tr><td>Precisión:</td><td>' + myPrecision + '</td></tr><tr><td>Velocidad:</td><td>' + speed + '</td></tr><tr><td>Dirección:</td><td>' + heading + '</td></tr><tr><td>Hora:</td><td>' + timestamp + '</td></tr></table>';
...
var failw = function(error) {
	...
	watchID = navigator.geolocation.getCurrentPosition(info, null,  options);
